...
public class JspV1InsertValuesTest {
  ...
  public void insertValues_thenCleanup() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection("jdbc:jimsql://127.0.0.1:8821/test?protocol=jspv1");
      ...
      int n1 = st.executeUpdate("INSERT INTO test.user (id,name,age) VALUES (999998,'tmp',1)");
      assertTrue(n1 >= 0);
      int n2 = st.executeUpdate("DELETE FROM test.user WHERE id = 999998");
      assertTrue(n2 >= 0);
    }
  }
}